...
contract BasicVickreyAuction {
    ...
    function calculateHash(bytes32 nonce, uint256 bidValue, uint256 tokenId, uint256 auctionIndex)
        external
        pure
        returns (bytes20)
    {
        bytes32 hash = keccak256(abi.encode(nonce, bidValue, tokenId, auctionIndex));
    ...
    function createAuction(
        ...
        require(bidEnd >= 1 minutes, "Bid period must be at least 1 minute");
        require(revealPeriod >= 1 minutes, "Reveal period must be at least 1 minute");

        auction.seller = payable(msg.sender);
        auction.bidStart = bidStart;
        auction.bidEnd = bidStart + bidEnd;
        auction.revealEnd = bidStart + bidEnd + revealPeriod;
    ...
    function commitBid(uint256 tokenId, bytes20 commitment) external payable {
        ...
        Bid storage bid = bids[tokenId][auctionIndex][msg.sender];
    ...
    function revealBid(uint256 tokenId, uint256 bidValue, bytes32 nonce) external {
        Auction storage auction = auctions[tokenId];
        ...
        bytes20 bidHash = bytes20(keccak256(abi.encode(nonce, bidValue, tokenId, auctionIndex)));
    ...
    function endAuction(uint256 tokenId) external returns (address) {
        ...
        if (highestBidder == address(0)) {
            ...
            Bid storage bid = bids[tokenId][auction.index][highestBidder];
            uint256 collateral = bid.collateral;
            bid.collateral = 0;
    ...
    function withdrawCollateral(uint256 tokenId, uint256 auctionIndex) external {
        Auction storage auction = auctions[tokenId];
    ...
    function getAuction(uint256 tokenId) external view returns (Auction memory auction) {
